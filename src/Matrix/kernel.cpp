...
//    Default constructor Creates a a two by two matrix with zeros in all
//    indexes
Matrix::Matrix()
{
  dim1 = 2;
...
/**
    Encodes an identiy matrix of dimension d1 and d2 - neglects lower rows if
   extra (0 rows)
...
/**
    Encodes an matrix of dimension d1 and d2 with random values all below
   desiredMax Used in initializing neural network - randomized matrices

    @param d1, d2 two dimensions of the matrix, rows x cols respectively and the
   max random value to the matrix
...
*/
void
Matrix::insert(int c1, int c2, float val)
{
  if (c1 >= dim1 || c2 >= dim2)
    return;
  arr[(dim2 * c1) + c2] = val;
...
*/
void
Matrix::replaceRow(int row, Matrix t)
{
  for (int i = 0; i < dim2; i++) {
...
*/
void
Matrix::replaceCol(int col, Matrix t)
{
  for (int i = 0; i < dim1; i++)
...
Matrix
Matrix::innerProd(Matrix tensor)
{
  Matrix newMatrix = Matrix(dim1, dim2);
  if (dim1 != tensor.dim1 || dim2 != tensor.dim2)
...
Matrix::sigmoidPrime()
{
  Matrix newTens = Matrix(dim1, dim2);
  for (int i = 0; i < dim1 * dim2; i++) {
    newTens.arr[i] = (1.0 / (1.0 + exp(1.0 * -arr[i]))) *
                     (1 - (1.0 / (1.0 + exp(1.0 * -arr[i]))));
  }
  return newTens;
}
...
/**
    rotates a vector by keeping the norm the same and appling a rotational
   matrix onto self

    @param t1, the matrix to rotate, theta how  many degrees, x, y, the two
   dimensions to rotate
    @return a new matrix  that is rotated by  theta degrees
*/
Matrix
rotate(Matrix t1, float theta, int x, int y)
{
...
Matrix
matlib::parMult(Matrix t, Matrix t2)
{
...
Matrix
matlib::parAdd(Matrix t, Matrix t2)
{
  Matrix t1 = t;
  if (t1.dim1 != t2.dim1)
...
Matrix
Matrix::principalEig(int max_iter)
{
...
// executes gaussjordan algorithm on self (1's down diagonal and upper
// traingular)
void
Matrix::GaussJordan()
{
