...
Network::Network(int numIn,
                 int numOut,
                 int numHiddenLayers,
                 int sizeHiddenLayers)
{
  ...
  while (temp->previous != nullptr) {
    temp->weights =
      Matrix(temp->next->size, temp->size, 1 / sqrt(float(numIn)));
...
// Prints the network (not to be confused with printNetworkSummary, this prints
// all the matrices)
void
...
/**
    ...
    @return a matrix after propogating a single layer (eventually the final
   output
*/
Matrix
propogateNetRecurs(layer* node, Matrix _inputs)
{
  node->inputs = _inputs;
...
*/
Matrix
Network::propogateNetwork(Matrix inputs)
{
  return propogateNetRecurs(input, inputs);
}

// prints a clean network summary including inputs, outputs, hidden layers and
// size hidden layers
void
...
/**
    ...
    @param expected a collumn vector with 1 in the spot  we want the vector to
   be
    @param rate the rate of training

    @return void recursively updates weights and biases using the chain rule in
   calculus
*/
void
Network::backPropogateRecurs(Matrix outputs, Matrix expected, float rate)
{
  ...
  temp->weights =
    temp->weights - ((parMult(temp->weights, delGrad)).scalarMult(rate));
