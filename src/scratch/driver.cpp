...
void
deleteMatrixMemory(MatrixMemory* m)
{
  while (m != nullptr) {
    MatrixMemory* temp = m;
    m = m->next;
...
void
deleteMatrixFromMemory(MatrixMemory* m, std::string id)
{
  while (m != nullptr) {
    if (m->id == id) {
      MatrixMemory* temp = m;
...
void
printMatrixMemory(MatrixMemory* m)
{
  while (m != nullptr) {
    printf("%s = \n", m->id.c_str());
    m->val.print();
...
void
printMatrix(MatrixMemory* m, std::string id)
{
  search(m, id).print();
}

Matrix
search(MatrixMemory* m, std::string id)
{
  while (m != nullptr) {
    if (m->id == id)
      return m->val;
