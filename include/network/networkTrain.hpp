...
namespace training {
struct trainElem
{ // a single training element (only stores  path to file, not actual matrix)
...
std::vector<trainElem>
uploadImages(int, std::string, std::string, int); // uploads 1 image
trainingSet
uploadAllImages(std::string, const char**, int*, int, std::string, int);
int getNumIn(cv::String);
matlib::Matrix
createOutput(int, int);
NetworkLib::Network*
trainDataSet(trainingSet, int, int, float);
bool
runOnMatrix(cv::String, NetworkLib::Network*, int);
float
testOnTestSet(trainingSet, NetworkLib::Network*);
}
